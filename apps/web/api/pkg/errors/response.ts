...
// 413
const contentTooLargeErrorSchema = errorSchema
  .extend({
    code: z.literal("CONTENT_TOO_LARGE").openapi({
      description: "The error code related to the status code.",
    }),
  })
  .openapi("ErrContentTooLarge");
...
export const contentTooLargeErrorResponse = {
  413: {
    description:
      "The request could not be completed due to its body content is too large",
    content: {
      "application/json": {
        schema: contentTooLargeErrorSchema,
      },
    },
  },
} satisfies ErrorResponse;
